...
function shouldForceGuidelineTool(messages) {
    const lastUser = [...messages]
        .reverse()
        .find((msg) => msg.role === 'user');
    if (!lastUser)
        return false;
    const text = extractTextFromMessageContent(lastUser.content).toLowerCase();
    if (!text)
        return false;
    const explicitNegative = /\b(?:não|nao|not)\b[^.!?\n]{0,24}\b(?:gerar|gere|generate|finalizar|concluir|criar)\b/i;
    if (explicitNegative.test(text))
        return false;
...
function compactFigmaContext(context, maxChars = MAX_CONTEXT_CHARS) {
...
function compactMessages(messages, maxMessages = MAX_MESSAGES) {
...
function buildSystemPrompt(figmaContext, forceGenerationNow) {
...
async function handler(req) {
    ...
    try {
        ...
        const { messages, figmaContext = '', requestId } = await req.json();
        ...
        const compactedMessages = compactMessages(messages, forceGuidelineTool ? MAX_MESSAGES_FOR_FORCED_GENERATION : MAX_MESSAGES);
        const compactedFigmaContext = compactFigmaContext(figmaContext, forceGuidelineTool ? MAX_CONTEXT_CHARS_FOR_FORCED_GENERATION : MAX_CONTEXT_CHARS);
